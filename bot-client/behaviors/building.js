...
async function buildStructure(bot, shape, material, dimensions, location) {
  ...
  // Snap to ground: find the first solid block below startPos
  for (let y = startPos.y - 1; y >= startPos.y - 10; y--) {
    const below = bot.blockAt(new Vec3(startPos.x, y, startPos.z));
    if (below && below.name !== 'air' && below.boundingBox === 'block') {
      startPos = new Vec3(startPos.x, y + 1, startPos.z);
      break;
    }
  }
